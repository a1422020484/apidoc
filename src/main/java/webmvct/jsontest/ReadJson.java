...
//	}
	...
	protected JSONObject tranJson(String address,String url) throws Exception{
        ...
        try {  
            ...
            while ((s = br.readLine()) != null) {  
                ...
                try {  
                    ...
                    definitionsMap = readJson.getDefinitions();
                    //request：dataJson未经修改的源数据。
                    //return ：dataJson中的definitions的引用已经替换了。
                    dataJson = readJson.tranDefinitions();
                    ...
                    for(String path : pathArray){
                    	//修改所有请求地址
//                    	if(url.equals(path)){
                    		dataJson = readJson.getKeyValueObject(pathsObj,path);
//                    	}
                    }
	...
	private JSONObject getKeyValueObject(JSONObject valueObjA,String key) throws Exception{
...
//					getKeyValue(valueObjB,keyStr);
	 ...
	 */
	private HashMap<String, Object> getDefinitions() {
	 ...
	 * @throws Exception 
	 */
	private JSONObject tranDefinitions() throws Exception{
		...
		for(String definitions: definitionsArray){
			...
			for(String propertiesName: propertiesNameArray){
				//会有异常
				dataJson = readJson.getKeyValueObject(propertiesObj,propertiesName);
			}
		}
		return dataJson;
	}
